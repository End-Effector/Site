...
//Este código já não está a ser usado, e tem de ser apagado.
//Ainda tem partes úteis.
function validateForm(form){ 
	...
	if(form.checkbox_1.checked) {
		if(form.iban.value == "" || form.address.value == ""){
			alert("If you want a detailed bill, you need to specify the iban and address.");
		...
		if(!re.test(form.iban.value)) {
			alert("Error in iban number.");
			form.iban.focus();
			return false;
		}
	...
	if(!re.test(form.postalcode.value)) {
		alert("Error in postalcode.");
		form.postalcode.focus();
		return false;
	}
	
	if(form.produto.value != "basic" && form.produto.value != "basicPlus") {
	...
	if(form.pwd1.value != "" && form.pwd1.value == form.pwd2.value) {
		...
		re = /^[\w]*$/;
			...
			if(!re.test(form.pwd1.value)) {
				alert("Error: password must contain at least one number, one lowercase,one uppercase letter and at least six characters");
				form.pwd1.focus();
				return false;
			}
	}else {
...
function validateShop(form){
	...
	if(form.checkbox_1.checked) {
		if(form.VatId.value == "" || form.address.value == ""){
			alert("If you want a detailed bill, you need to specify the Vat. Identification number and address.");
			form.checkbox_1.focus();
			return false;
		}
		...
		if(!re.test(form.VatId.value)) {
			alert("Error in Vat Identification number.");
			form.iban.focus();
			return false;
		}
...
function validateUser(form){
	...
	if(!re.test(form.workouts.value)) {
		alert("Error in first name.");
		form.workouts.focus();
		return false;
	}
	...
	if(!re.test(form.other.value)) {
		alert("Error in first name.");
		form.other.focus();
		return false;
	}
	
	if(!re.test(form.health.value)) {
		alert("Error in first name.");
		form.health.focus();
		return false;
	}
	
	if(!re.test(form.supplements.value)) {
		alert("Error in first name.");
		form.supplements.focus();
		return false;
	}
	
	var re = /^[\s\w\d]*$/;
	
	if(!re.test(form.contact_text.value)) {
		alert("Error in first name.");
		form.contact_text.focus();
		return false;
	}
...
/*Para a barra nav abrir quando a pagina se encontra em modo telemovel.*/
